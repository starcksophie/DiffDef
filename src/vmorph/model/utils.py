...
# from data.datasets import BrainMRInterSubj3D, CardiacMR2D, ContrastiveImagingAndECGDataset
...
def get_network(hparams):
    ...
    if hparams.network.type == "unet":
        network = UNet(ndim=hparams.data.ndim,
                       pretrained=hparams.network.pretrained,
...
def get_loss_fn(hparams):
    ...
    elif hparams.loss.sim_loss == 'dice':
        sim_loss_fn = loss.Dice()
    else:
...
#     elif hparams.data.name == 'cardiac_ecg':
#         train_dataset = ContrastiveImagingAndECGDataset(hparams.data.train_path, hparams.data.train_seg_path,delete_segmentation=hparams.data.del_seg, img_size=hparams.data.img_size)
#         val_dataset = ContrastiveImagingAndECGDataset(hparams.data.val_path, hparams.data.val_seg_path, delete_segmentation=hparams.data.del_seg, img_size=hparams.data.img_size)
