...
from model.lightning_module import LightningDLReg
...
def inference(model, dataloader, output_dir, device=torch.device('cpu')):
    for idx, batch in enumerate(tqdm(dataloader)):
        ...
        # model inference
        out = model(batch['fixed'], batch['moving'])
        batch['disp_pred'] = out[1] if len(out) == 2 else out  # (flow, disp) or disp

        # warp images and segmentation using predicted disp
        batch['warped_moving'] = warp(batch['moving'], batch['disp_pred'])
        if 'moving_seg' in batch.keys():
            batch['warped_moving_seg'] = warp(batch['moving_seg'], batch['disp_pred'],
                                              interp_mode='nearest')
        if 'fixed_original' in batch.keys():
            batch['fixed_pred'] = warp(batch['fixed_original'], batch['disp_pred'])
