...
def visualise_results(fixed_image, moving_image, transformed_image, field, title="Validation intensities", cmap='viridis'):
    ...
    for a in range(0, 3):
        ...
        fixedAx = np.rot90(torch.index_select(fixed_image.cpu().clone().reshape(image_size), dim=a, index=torch.tensor([z])).squeeze().numpy(), k=2)
        movingAx = np.rot90(torch.index_select(moving_image.clone().cpu().reshape(image_size), dim=a, index=torch.tensor([z])).squeeze().numpy(), k=2)
        warpedAx = np.rot90(torch.index_select(transformed_image.detach().cpu().clone().reshape(image_size), dim=a, index=torch.tensor([z])).squeeze().numpy(), k=2)
        fieldAx = np.rot90(torch.index_select(field[axes, ...], dim=a+1, index=torch.tensor([z])).squeeze().numpy(), k=2)
        ...
        plot_warped_grid(ax[a, 5], fieldAx, None, interval=4, title=f"axis {a}", fontsize=20)
